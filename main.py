...
@app.route(’/’)
def health():
return jsonify({“status”: “ok”, “server”: “MESH SPX Server”})
...
if rank < 20:
return {‘en’: ‘Do Not Trade’, ‘ar’: ‘لا تتداول اليوم’}
if rank < 40:
return {‘en’: ‘Credit Spread Narrow’, ‘ar’: ‘سبريد ائتماني ضيق’}
if rank < 60:
return {‘en’: ‘Iron Condor’, ‘ar’: ‘آيرون كوندور’}
if rank < 80:
return {‘en’: ‘Iron Condor Wide’, ‘ar’: ‘آيرون كوندور واسع’}
...
‘direction’: ‘bullish’,
‘strength’: 65,
‘reason’: ‘السعر فوق Zero-Gamma (’ + str(gex[‘zeroGamma’]) + ‘). GEX ايجابي.’
}
if not above_zg and neg_gex:
return {
‘direction’: ‘bearish’,
‘strength’: 68,
‘reason’: ‘السعر تحت Zero-Gamma (’ + str(gex[‘zeroGamma’]) + ‘). GEX سلبي.’
}
return {
‘direction’: ‘neutral’,
‘strength’: 42 if vix > 25 else 52,
‘reason’: ‘بين Put Wall (’ + str(gex[‘putWall’]) + ‘) و Call Wall (’ + str(gex[‘callWall’]) + ‘).’
}
...
‘weeklyUpper’: price + w,
‘weeklyLower’: price - w,
‘expDate’: fri.strftime(’%Y-%m-%d’)
}

def get_scenarios(price, gex, direction):
cw = gex[‘callWall’]
pw = gex[‘putWall’]
mp = gex.get(‘maxPain’, round(price / 25) * 25)
zg = gex[‘zeroGamma’]
if direction == ‘bearish’:
return [
{‘type’: ‘bearish’,   ‘icon’: ‘down’, ‘name’: ‘Bearish Continuation’, ‘nameAr’: ‘هبوط مستمر’,        ‘prob’: 55, ‘target’: pw, ‘stop’: zg},
{‘type’: ‘neutral-s’, ‘icon’: ‘side’, ‘name’: ‘Range-Bound’,          ‘nameAr’: ‘تداول في نطاق’,      ‘prob’: 30, ‘target’: mp, ‘stop’: round(pw * 0.995)},
{‘type’: ‘bullish’,   ‘icon’: ‘up’,   ‘name’: ‘Bullish Reversal’,     ‘nameAr’: ‘انعكاس صاعد’,        ‘prob’: 15, ‘target’: zg, ‘stop’: pw},
]
if direction == ‘bullish’:
return [
{‘type’: ‘bullish’,   ‘icon’: ‘up’,   ‘name’: ‘Bullish Breakout’, ‘nameAr’: ‘اختراق صعودي’,   ‘prob’: 50, ‘target’: cw, ‘stop’: zg},
{‘type’: ‘neutral-s’, ‘icon’: ‘side’, ‘name’: ‘Range-Bound’,      ‘nameAr’: ‘تداول في نطاق’,  ‘prob’: 35, ‘target’: mp, ‘stop’: pw},
{‘type’: ‘bearish’,   ‘icon’: ‘down’, ‘name’: ‘Bearish Flush’,    ‘nameAr’: ‘هبوط نحو الدعم’, ‘prob’: 15, ‘target’: pw, ‘stop’: cw},
]
return [
{‘type’: ‘neutral-s’, ‘icon’: ‘side’, ‘name’: ‘Range-Bound’,    ‘nameAr’: ‘تداول في نطاق محدد’, ‘prob’: 55, ‘target’: mp, ‘stop’: round(pw * 0.995)},
{‘type’: ‘bullish’,   ‘icon’: ‘up’,   ‘name’: ‘Bullish Breakout’,‘nameAr’: ‘اختراق صعودي’,       ‘prob’: 25, ‘target’: cw, ‘stop’: zg},
{‘type’: ‘bearish’,   ‘icon’: ‘down’, ‘name’: ‘Bearish Flush’,   ‘nameAr’: ‘تصحيح هبوطي’,        ‘prob’: 20, ‘target’: pw, ‘stop’: round(price * 1.005)},
]

def get_decision(bias, gex, price, rank):
if rank < 20:
return {‘action’: ‘لا تتداول اليوم’, ‘detail’: ‘IV منخفض جدا’}
dc = gex[‘callWall’] - price
dp = price - gex[‘putWall’]
if bias == ‘bearish’ and dc < dp * 1.5:
return {
‘action’: ‘SELL CALL Credit Spread’,
‘detail’: ’الدخول عند ’ + str(round(gex[‘zeroGamma’])) + ’ الجدار ’ + str(gex[‘callWall’])
}
if bias == ‘bullish’ and dp < dc * 1.5:
return {
‘action’: ‘SELL PUT Credit Spread’,
‘detail’: ’الدخول عند ’ + str(round(gex[‘zeroGamma’])) + ’ الدعم ’ + str(gex[‘putWall’])
}
return {
‘action’: ‘Iron Condor’,
‘detail’: ’بيع بين ’ + str(gex[‘putWall’]) + ’ و ’ + str(gex[‘callWall’])
}
...
{‘date’: ‘2026-04-08’, ‘name’: ‘Initial Jobless Claims’,     ‘nameAr’: ‘طلبات الاعانة’,          ‘impact’: ‘MEDIUM’, ‘est’: ‘222K’, ‘prev’: ‘219K’},
